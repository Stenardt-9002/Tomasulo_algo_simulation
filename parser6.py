        ...
        if trarvar>=CLOCK_VAR:
            ...
            if opc_odes1[trarvar]=="1000":
                ...
                if instruction_status[trarvar]['started'] == 0:
                    ...
                    if continue_WAR[0] and continue_WAW_var_allowed:#allowed register renaming
                        # print("DEBUGE2")
                        if len(continue_WAR)==4:
                    ...
                    if instruction_status[trarvar]['completed_yet'] ==1:
                        ...
                        if temp_passvar :
                            ...
                            if ((isinstance(register_Status[res_totherinputs[trarvar][0]],list)) and (register_Status[res_totherinputs[trarvar][0]][1]==trarvar)) or (isinstance(register_Status[res_totherinputs[trarvar][0]],int) and ( register_Status[map_table[res_totherinputs[trarvar][0]][0]][1]==trarvar  )):
                                ...
                                if reservation_Station[instruction_status[trarvar]['key']]['wb']['busy'] == 1 and reservation_Station[instruction_status[trarvar]['key']]['wb']['op']==trarvar:#only that hardware
                                    ...
                                    # reservation_Station[instruction_status[trarvar]['key']]['wb']['busy'] = 1 #now busy
                                    # reservation_Station[instruction_status[trarvar]['key']]['ex']['busy'] = 0 #freed
                                    # reservation_Station[instruction_status[trarvar]['key']]['wb']['op'] = reservation_Station[instruction_status[trarvar]['key']]['ex']['op']
                                    # reservation_Station[instruction_status[trarvar]['key']]['wb']['vj'] = reservation_Station[instruction_status[trarvar]['key']]['ex']['vj']
                                    # reservation_Station[instruction_status[trarvar]['key']]['wb']['vk'] = reservation_Station[instruction_status[trarvar]['key']]['ex']['vk']
                                    # reservation_Station[instruction_status[trarvar]['key']]['wb']['qj'] = reservation_Station[instruction_status[trarvar]['key']]['ex']['qj']
                                    # reservation_Station[instruction_status[trarvar]['key']]['wb']['qk'] = reservation_Station[instruction_status[trarvar]['key']]['ex']['qk']
                                    #
                                    #
                                    # reservation_Station[instruction_status[trarvar]['key']]['ex']['op'] = -1
                                    # reservation_Station[instruction_status[trarvar]['key']]['ex']['vj'] = 0
                                    # reservation_Station[instruction_status[trarvar]['key']]['ex']['vk'] = 0
                                    # reservation_Station[instruction_status[trarvar]['key']]['ex']['qj'] = 0
                                    # reservation_Station[instruction_status[trarvar]['key']]['ex']['qk'] = 0
                        ...
                        if reg1.isdigit()==False:
                            ...
                            pass
                                ...
                                # print(res_totherinputs[trarvar][1])
                                # print(register_Status[res_totherinputs[trarvar][1]])
                                # temp_passvar = (temp_passvar2 and (register_Status[res_totherinputs[trarvar][1]][1]==trarvar))
                    ...
                    if instruction_status[trarvar]['completed_yet'] == 0:
                        ...
                        if res_totherinputs[trarvar][0] in map_table.keys():
                            ...
                            if register_Status[res_totherinputs[trarvar][0]]!=0:
                                ...
                                if register_Status[map_table[res_totherinputs[trarvar][0]][0]][1] == trarvar:
                                    temmmpempstr  = "BUFFER TRANSFER FROM to "+str(map_table[res_totherinputs[trarvar][0]][0])
                                    RAISEEXCEPTION.append(temmmpempstr)
...
#all instruction except 2 input
            ...
            if instruction_status[trarvar]['started'] == 0:
                #not this is only issue therfore no need to check if harware available or not just issue it
                #check if hardware available
                # if True:
                # if register_Status[res_totherinputs[trarvar][1]]==0 && register_Status[res_totherinputs[trarvar][2]]==0:

                #Check hardware for issue #assumption only one hardware
                # if reservation_Station[instruction_status[trarvar]['key']]['busy']==0 and reservation_Station[instruction_status[trarvar]['key']]['issueavailable']==0:#notbusy   then can only be issue done

                # if res_totherinputs[trarvar][1].isdigit() == False and res_totherinputs[trarvar][2].isdigit() == False:
                    # print(res_totherinputs[trarvar][1])
                    # print(res_totherinputs[trarvar][2])
                    # print((type)(res_totherinputs[trarvar][1]))
                    # print((type)(res_totherinputs[trarvar][2]))
                    # print(register_Status[res_totherinputs[trarvar][1]])
                    # print(register_Status[res_totherinputs[trarvar][2]])
                    # print((type)(register_Status[res_totherinputs[trarvar][1]]))
                    # print((type)(register_Status[res_totherinputs[trarvar][2]]))
                ...
                #
                #         # temp_passvar = (register_Status[res_totherinputs[trarvar][1]]==0 and register_Status[res_totherinputs[trarvar][2]]==0)or(register_Status[res_totherinputs[trarvar][1]][1]==trarvar and register_Status[res_totherinputs[trarvar][2]][1]==trarvar)
                #     else:
                #         temp_passvar = False

                # if temp_passvar:
                # prevent WAr HAZARD
                # contrinue WAR BUT register renaming
                # x1 = check_War(reservation_Station,res_totherinputs[trarvar][0],trarvar)
                # print(x1)
                # print(type(x1))
                # x2 = input()
                continue_WAW_var_allowed = True
                ...
                if continue_WAR[0] and continue_WAW_var_allowed:#allowed register renaming
                    ...
                    # else:
                    #     if register_Status[res_totherinputs[trarvar][1]]!=0:
                    #
                    #
                    #         pass
                    #     elif register_Status[res_totherinputs[trarvar][2]]!=0:
                    #         pass
                    #
                ...
                if instruction_status[trarvar]['completed_yet'] ==1:
                    ...
                    if (isinstance((register_Status[res_totherinputs[trarvar][1]]),list) and isinstance((register_Status[res_totherinputs[trarvar][2]]),list)):
                        ...
                        if (isinstance((register_Status[res_totherinputs[trarvar][1]]),int) and isinstance((register_Status[res_totherinputs[trarvar][2]]),int)):
                            ...
                            # temp_passvar = (register_Status[res_totherinputs[trarvar][1]]==0 and register_Status[res_totherinputs[trarvar][2]]==0)or(register_Status[res_totherinputs[trarvar][1]][1]==trarvar and register_Status[res_totherinputs[trarvar][2]][1]==trarvar)
                            # if res_totherinputs[trarvar][1] in map_table.keys() or res_totherinputs[trarvar][2] in map_table.keys():
                            #     # print("reached herer",res_totherinputs[trarvar][1],res_totherinputs[trarvar][2])
                            #     temp_passvar = False
                        else:
                            temp_passvar = False
    ...
    for keys in get_hardwareused_list:
        if keys in ["LOAD1","STORE","REGISTER_COMPLEMENT"]:
            ...
            print('qj ',reservation_Station[keys]['is']["qj"])
            ...
            print('qj ',reservation_Station[keys]['ex']["qj"])
            ...
            print('qj ',reservation_Station[keys]['wb']["qj"])
    ...
    for keyuar in map_table.keys():
        if isinstance(register_Status[map_table[keyuar][0]],list):
            print("Instruction ",register_Status[map_table[keyuar][0]][1]+1,"   ",keyuar," is renamed to ",map_table[keyuar][0] )
    print()
    print("\nmap_table\n")
    print(map_table)
    print()
    ...
    #free map table registerstatus placement




    # for keyvar in register_Status.keys():
    #     if keyvar not in temp_ereg:
    #         if keyvar in map_table.keys():#occupied
    #             ope_index = register_Status[map_table[keyvar][0]][1]
    #             if instruction_status[ope_index]['completed_yet'] == -1:
    #                 #copy theoretical value of placement to original
    #                 # map_table[keyvar] = 0
    #                 # if register_Status[opco]
...
#ex to free stage
    ...
    #
    # for eachunit in reservation_Station.keys():
    #     if reservation_Station[eachunit]['wb']['busy'] == 1:
    #
    #         reservation_Station[eachunit]['wb']['busy'] = 0
    #
    #         reservation_Station[eachunit]['wb']['op'] = -1
    #         reservation_Station[eachunit]['wb']['vj'] = 0
    #         reservation_Station[eachunit]['wb']['vk'] = 0
    #         reservation_Station[eachunit]['wb']['qj'] = 0
    #         reservation_Station[eachunit]['wb']['qk'] = 0
